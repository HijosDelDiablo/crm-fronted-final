...
export default function Catalogo() {
  ...
  const filteredProductos = productos.filter((auto) => {
    const matchSearch = `${auto.marca} ${auto.modelo}`
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchCondicion =
      filterCondicion === "Todos" || auto.condicion === filterCondicion;
  ...
  const handleCotizar = async () => {
    ...
    try {
      ...
      if (modalType === "cotizar") {
        toast.success("¡Cotización enviada! Revisa tu correo.", {
          duration: 5000,
        });
        handleClose();
      }
  ...
  const handleComprar = async () => {
    ...
    try {
      ...
      toast.success("Solicitud enviada. Consultaremos tu Buró.", {
        duration: 6000,
      });
      handleClose();
    } catch {
  ...
  const openModal = (auto, type) => {
    ...
    setModalType(type);
    setEnganche(auto.precioBase * 0.2);
  };
    ...
    <Container fluid className="py-4 catalog-container">
      <div className="catalog-header mb-4">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div>
            <h2 className="fw-bold text-dark mb-1 catalog-title">
              Vehículos Disponibles
            </h2>
            <p className="text-muted mb-0">
              {filteredProductos.length} vehículos encontrados
            </p>
          </div>
            ...
            <div className="filter-buttons">
              {["Todos", "Nuevo", "Seminuevo"].map((cond) => (
                <Button
                  key={cond}
                  variant={
                    filterCondicion === cond ? "primary" : "outline-secondary"
                  }
                  size="sm"
          ...
          <Col key={auto._id} md={6} lg={4} xl={3}>
            <div className="product-card h-100">
              ...
              <div className="product-body">
                ...
                <h4 className="price-tag mb-3">
                  ${auto.precioBase.toLocaleString()}
                ...
                <div className="product-specs mb-3">
                  <div className="spec-item">
                    <Car size={14} />
                    <small>{auto.transmision}</small>
                  </div>
                  <div className="spec-item">
                    <Calendar size={14} />
                    <small>{auto.kilometraje.toLocaleString()} km</small>
                  </div>
                </div>
                ...
                <div className="product-actions">
                  <Button
                    ...
                    onClick={() => openModal(auto, "detalle")}
                  >
                    <Info size={18} />
                    <span>Detalles</span>
                  </Button>
                  ...
                  <div className="d-flex gap-2 flex-fill">
                    <Button
                      ...
                      className="btn-action flex-fill"
                    ...
                    <Button
                      variant="primary"
                      className="btn-action flex-fill"
                      onClick={() => openModal(auto, "comprar")}
                    >
            ...
            {modalType === "detalle" && (
              <div>
                ...
                <Row className="g-3">
                  <Col md={6}>
                    <p>
                      <strong>Condición:</strong> {selectedAuto.condicion}
                    </p>
                    <p>
                      <strong>Año:</strong> {selectedAuto.ano}
                    </p>
                    <p>
                      <strong>Transmisión:</strong> {selectedAuto.transmision}
                    </p>
                  </Col>
                  <Col md={6}>
                    <p>
                      <strong>Kilometraje:</strong>{" "}
                      {selectedAuto.kilometraje.toLocaleString()} km
                    </p>
                    <p>
                      <strong>Color:</strong> {selectedAuto.color}
                    </p>
                    <p>
                      <strong>Motor:</strong> {selectedAuto.motor}
                    </p>
                  </Col>
                </Row>
                ...
                {enganche > 0 && (
                  <div className="bg-light p-3 rounded border mt-2">
                    ...
                    <p className="fw-bold text-primary">
                      ...
                      {(
                        (selectedAuto.precioBase - enganche) /
                        plazo
                        ...
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            onChange={(e) =>
                              setFinancialData({
                                ...financialData,
                                gastosMensuales: e.target.value,
                              })
                            }
                          />
                        </Form.Group>
