...
export const getCotizacionesPendientes = async (navigate) => {
    const response = await fetchApiGet('/cotizacion/pendientes', navigate, 'Error al obtener cotizaciones pendientes');
    return response;
};

export const getCotizacionesAprobadas = async (navigate) => {
    const response = await fetchApiGet('/cotizacion/aprobadas', navigate, 'Error al obtener cotizaciones aprobadas');
    return response;
};
